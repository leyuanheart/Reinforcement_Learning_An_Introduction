# -*- coding: utf-8 -*-
"""
Created on Thu Mar 11 09:17:11 2021
...
# additional setting for exercise 4.7
second_parking_lot_cost = 4
# One of Jack’s employees is willing to move a car from first loc to second loc for free
# so if action is "+", the total moving cost will become (action - 1) * move_cost
...
def expected_return(state, action, value, constant_returned_cars=True):
    ...
    if action > 0:
        total_moving_cost = (action - 1) * move_cost
    else:
        total_moving_cost = abs(action) * move_cost
    
    
    expected_return -= total_moving_cost
    
    # moving cars 一定要在go through 外面定义实际的car数，go through里面是模拟所有可能的情况
    NUM_OF_CARS_FIRST_LOC = min(state[0] - action, max_cars)
    NUM_OF_CARS_SECOND_LOC = min(state[1] + action, max_cars)
    
    # compute the addtitonal parking fee
    add_parking_cost = (second_parking_lot_cost if NUM_OF_CARS_FIRST_LOC > 10 else 0) + \
                       (second_parking_lot_cost if NUM_OF_CARS_SECOND_LOC > 10 else 0)
    expected_return -= add_parking_cost
...
def draw_fig(value, policy, iteration):
    ...
    plt.savefig(f'iteration {iteration}.png', bbox_inches='tight')
